...
class Requirements {
	...
	public function __construct( $plugin_name = '', $to_check = array() ){
		...
		// Add default checks
		$this->add_check( 'php', array( $this,
										'check_php',
		) );
		$this->add_check( 'php_extensions', array( $this,
												   'check_php_extensions',
		) );
		$this->add_check( 'wp', array( $this,
									   'check_wp',
		) );
		$this->add_check( 'plugins', array( $this,
											'check_plugins',
		) );
		$this->add_check( 'theme', array( $this,
										  'check_theme',
		) );
		$this->add_check( 'function_collision', array( $this,
													   'check_function_collision',
		) );
		$this->add_check( 'class_collision', array( $this,
	...
	public function check_php_extensions( $extensions, $requirements ){
		...
		if( !empty( $missing_extensions ) ){
			$requirements->add_error( sprintf( _n( 'PHP extension: %s', 'PHP extensions: %s', count( $missing_extensions ) ), implode( ', ', $missing_extensions ) ) );
		}
	...
	public function check_function_collision( $functions, $requirements ){
		...
		if( !empty( $collisions ) ){
			$requirements->add_error( sprintf( _n( 'register %s function but it\'s already taken', 'register %s functions but these are already taken', count( $collisions ) ), implode( ', ', $collisions ) ) );
		}
	...
	public function check_class_collision( $classes, $requirements ){
		...
		if( !empty( $collisions ) ){
			$requirements->add_error( sprintf( _n( 'register %s class but it\'s already defined', 'register %s classes but these are already defined', count( $collisions ) ), implode( ', ', $collisions ) ) );
		}
